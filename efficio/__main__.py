...
def find_subclasses_in_directory(
    target_class: Type[Any], directory: str
) -> List[Type[Any]]:
    subclasses = []
...
def fetch_object_data() -> Dict[Type[Any], Dict[str, Tuple[str, type]]]:
    ...
    for subclass in subclasses:
        ...
        param_dict = {
            class_param: class_params[class_param] for class_param in class_params
        }
...
def print_valid_classes(
    message: str,
    options: Dict[Type[Any], Dict[str, Tuple[str, type]]],
    only_class: str = "",
) -> None:
    if message:
        sys.stderr.write(f"\nError: {message}\n\n")
        sys.stderr.write(
            "Please provide the required arguments as shown in the usage message above.\n"
        )
...
def main(
    obj_name: str,
    obj_params: Optional[List[Tuple[str, str]]],
        ...
        if option_type_str == "Measure":
            ...
            if issubclass(option_type, Enum):
                if option_value not in option_type.__members__:
                    print_valid_classes(
                        f"{option_name} is not a valid {option_type_str} value. Valid values are: {list(option_type.__members__.keys())}",
                        options,
                    )
                    sys.exit(5)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--object", type=str, required=True, help="The name of the object"
    )
    parser.add_argument(
        "--params",
        action=parse_obj_params,
        help="Key-value pair parameters in the form key=value",
    )
